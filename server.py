...
*** PROTOCOL ***
1. CHART: Output JSON in <<<CHART_START>>> ... <<<CHART_END>>>.
2. PPT/DOC: Output content in <<<FILE_START_PPTX>>> or <<<FILE_START_DOCX>>> tags.
3. IMAGE: Reply "IMAGE_GEN_REQUEST: [Detailed Prompt]"
4. DEFAULT: Answer normally. Save facts with <<<MEM_SAVE>>>.
"""
...
def generate_image(prompt):
    if not HUGGINGFACE_API_KEY: return None, "No API Key."
    ...
    for m in models:
        try:
            ...
            r = requests.post(m, headers=headers, json={"inputs": prompt}, timeout=30)
            if r.status_code == 200 and "image" in r.headers.get("content-type", ""):
                return base64.b64encode(r.content).decode('utf-8'), "gen.jpg"
        except: continue
    return None, "Servers Busy"
...
async def process_command(request: UserRequest, x_alfred_auth: Optional[str] = Header(None)):
    ...
    try:
        ...
        if "IMAGE_GEN_REQUEST:" in response:
            ...
            else: response = "Visual Engine Offline: Server Busy."
